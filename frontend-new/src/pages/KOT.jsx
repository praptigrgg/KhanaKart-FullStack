...
import { Printer, Clock, CheckCircle, Eye, Filter } from 'lucide-react';
  ...
  const applyFilters = () => {
    ...
    if (tableFilter.trim() !== '') {
      result = result.filter(order =>
        order.table?.table_number?.toString().includes(tableFilter.trim())
      );
    }
  ...
  const generateKOTHTML = (order) => `
    <html>
      <head>
        ...
        <style>
          body { font-family: 'Courier New', monospace; margin: 0; padding: 20px; font-size: 14px; line-height: 1.4; }
          .header { text-align: center; margin-bottom: 20px; border-bottom: 2px solid #000; padding-bottom: 10px; }
          .order-info { margin-bottom: 20px; display: flex; justify-content: space-between; }
          .items { margin-bottom: 20px; }
          .item { display: flex; justify-content: space-between; margin-bottom: 5px; padding: 5px 0; border-bottom: 1px dashed #ccc; }
          .item-name { font-weight: bold; }
          .footer { margin-top: 20px; text-align: center; border-top: 2px solid #000; padding-top: 10px; }
          .status { background: #f0f0f0; padding: 5px 10px; border-radius: 3px; display: inline-block; }
        </style>
      </head>
      <body>
        <div class="header">
        ...
        <div class="order-info">
          ...
          <div><span class="status">${order.status.toUpperCase()}</span></div>
        </div>
        <div class="items">
          ...
          ${order.items?.map(item => `
            <div class="item">
              ...
              <div><span class="status">${item.status?.toUpperCase() || 'PENDING'}</span></div>
            </div>
          `).join('') || '<p>No items found</p>'}
        </div>
        <div class="footer">
          <p><strong>Total Items:</strong> ${order.items?.reduce((sum, i) => sum + i.quantity, 0) || 0}</p>
      ...
      {showModal && selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4">
              <p><strong>Table:</strong> {selectedOrder.table?.table_number || 'N/A'}</p>
              <p><strong>Order Time:</strong> {format(new Date(selectedOrder.created_at), 'MMM dd, yyyy HH:mm')}</p>
              <p>
            ...
            <div>
              ...
              <ul className="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded p-3">
                {selectedOrder.items?.map((item, idx) => (
                  <li key={idx} className="flex justify-between items-center border-b border-gray-100 pb-1 last:border-b-0">
                    <h4 className="font-medium text-gray-900">
                      {item.menuItem?.name || item.menu_item?.name || 'Unknown Item'}
                    </h4>
                    <span className="font-semibold">x{item.quantity}</span>
                    <span className={`text-xs px-2 py-0.5 rounded ${getStatusColor(item.status || 'pending')}`}>
                      {item.status || 'pending'}
                    </span>
                  </li>
                ))}
