   ...
   private function cashierDashboard($user)
{
    ...
    // Get today's orders with items and menu items
    $ordersToday = Order::with('items.menuItem')
        ->whereDate('created_at', $today)
        ->get();

    // Split paid and unpaid orders
    $paidOrdersToday = $ordersToday->filter(fn($order) => $order->is_paid);
    $pendingPaymentsToday = $ordersToday->filter(fn($order) => !$order->is_paid);

    // Calculate total revenue safely
    $totalRevenueToday = $paidOrdersToday->reduce(function ($carry, $order) {
        $orderTotal = $order->items->sum(function ($item) {
            return optional($item->menuItem)->price * $item->quantity;
        });
    ...
    // Monthly revenue
    $monthStart = now()->startOfMonth();
    $monthOrders = Order::with('items.menuItem')
    ...
    return response()->json([
        ...
        'paid_orders_today' => $paidOrdersToday->count(),
        'pending_payments_today' => $pendingPaymentsToday->count(),
        'total_revenue_today' => round($totalRevenueToday, 2),
        'monthly_revenue' => round($monthlyRevenue, 2),
        'recent_orders' => $ordersToday->sortByDesc('created_at')->take(5)->values(), // safe ordering
    ]);
}
